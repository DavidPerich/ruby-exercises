...
# returns thee elemnt for which the block returns the largest value. 
# input an array. output the largest element. If empty return nil. 

def max_by(array) 
  value = array.first
  array.each {|item| value = item if yield(item) > yield(value)}
  value
end 
...
#     max_element = item if yield(item) > yield(max_element)
#   end
#   max_element
# end 