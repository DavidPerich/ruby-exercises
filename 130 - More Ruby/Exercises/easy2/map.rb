...
# It should take an Array as an argument, and a block. It should return a new Array that contains the return values produced by the block for each element of the original Array.
...
def map(collection)
  collection.each.with_object([]) do |item,results|
    results << yield(item)
  end
end 
